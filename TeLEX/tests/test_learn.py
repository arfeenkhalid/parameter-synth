...
def test_stl(tlStr):
    print(tlStr)
    try:
        (stlsyn, value, dur) = TeLEX.telex.synth.synthSTLParam(tlStr, "traces")
    except ValueError:
        (stlsyn, value, dur) = TeLEX.telex.synth.synthSTLParam(tlStr, "traces", "nogradient")
    print(" Synthesized STL formula: {}\n Theta Optimal Value: {}\n Optimization time: {}\n".format(stlsyn, value, dur))
    (bres, qres) = TeLEX.telex.synth.verifySTL(stlsyn, "traces")
